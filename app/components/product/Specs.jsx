...
class Specs extends Component {
    constructor(props) {
        super(props);
        this.state = {
            id: this.props.productTemplateId,
            visible: true,
            placement: 'right',
            three: {}
        }
    ...
    loadData = (id) => {
        ...
        App.api(`adm/product/product_template_three`, {
            id
        }).then((three) => {
            this.setState({
                three,
    ...
    render() {
        ...
        let { productTemplate = {}, pre = {}, next = {} } = three
        let { name, specs = [], params = [], content, id } = productTemplate
        return <Drawer title={name}
            ...
            <Tabs>
                {specs.map((spec, index) => {
                    let { imgs = [], params = [], price, sno } = spec;
                    ...
                    return <TabPane tab={`规格 ${index + 1}`} key={index} closable={index > 0}  >
                        ...
                        <Descriptions layout="horizontal">
                            ...
                            <Descriptions.Item key={sno} label="库存">
                                {sno}
                            </Descriptions.Item>
                            <Descriptions.Item key={id} label="编号">
                                {id}
                            </Descriptions.Item>
                        </Descriptions>
                    </TabPane>
