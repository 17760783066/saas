...
import { Button, Card, Input, Modal, message,Select } from 'antd';
...
class UiEdit extends Component {
    ...
    show = (common, index, key, val) => {
        ...
        this.setState({
            ...
            if (['BANNER', 'AD', 'NAV', 'TOP', 'COURSE', 'TRAINER', 'LIVE', 'ARTICLE', 'PUTAWAY'].includes(key) && val) {
    ...
    filterHomeModules = () => {
        ...
        homeComps.map((m, i) => {
            ...
            m.withTitle = ['TOP', 'COURSE', 'TRAINER', 'LIVE', 'ARTICLE', 'PUTAWAY'].includes(m.key);
            m.withMore = ['TOP', 'COURSE', 'TRAINER', 'LIVE', 'ARTICLE', 'PUTAWAY'].includes(m.key);
    ...
    render() {
        ...
        let isPutaway = currModuleKey === 'PUTAWAY';
        let isDecoration = currModuleKey === 'DECORATION';
        let isAfflatus = currModuleKey === 'AFFLATUS';
                ...
                <Card extra={<Button type="primary" icon={<FileAddOutlined />} loading={this.state.loading}
                    ...
                    {currModuleIndex > -1 && <Card title={currItem.name} style={{ width: '400px', float: 'left' }}>
                        <div className='common-module-form'>
                            ...
                            <div className='line'>
                                <Button type="primary" icon={<FileAddOutlined />} onClick={() => {
                                    if (isBanner || isAd) {
                                        ...
                                        SettingUtils.merchantPicker(list, this.syncItems, true);
                                    } else if (isCourse) {
                                        ...
                                        SettingUtils.putawayPicker(list, this.syncItems, true);
                                    } else if (isTop) {
                                        SettingUtils.productPicker(list, this.syncItems, true);
                                    }else if (isDecoration) {
                                        SettingUtils.decorationPicker(list, this.syncItems, true);
                                    }else if (isAfflatus) {
                                        SettingUtils.afflatusPicker(list, this.syncItems, true);
                            ...
                            {(isBanner || isAd) &&
                                <ul className='banners' id={`item_sorter_${currModuleIndex}`}>
                                    {list.map((b, i) => {
                                        console.log(b);
                                            ...
                                            <div className='img'><img src={b.img} />
                                                ...
                                                <i className='edit'
                                                    onClick={() => {
                                                        ...
                                                        SettingUtils.bannerEdit(b, this.syncBanner);
                                                    }} />
                                            </div>
                                            <p>
                            ...
                            {((isNav) || isTrainer) &&
                                <ul className='banners categories' id={`item_sorter_${currModuleIndex}`}>
                                    {list.map((b, i) => {
                                        let li = { width: isNav && type === 2 ? '180px' : '110px' };
                                        let width = { width: isNav && type === 2 ? '160px' : null };
                                        return <li style={li} key={i} data-id={JSON.stringify(b)}>
                                            <div className='img' style={width}>
                                                <img style={width}
                            ...
                            {isCourse && <ul className='products' id={`item_sorter_${currModuleIndex}`}>
                                {list.map((b, i) => {
                                    let { name, course = {} } = b;
                                    return <li key={i} data-id={JSON.stringify(b)}>
                                        <img
                                            src={(course.imgs && course.imgs[0])} />
                                        <div className='title'>
                                            {name}
                                        </div>
                            ...
                            {
                                isPutaway && <ul className='products' id={`item_sorter_${currModuleIndex}`} >
                                    {list.map((b, i) => {
                                        let { imgs = {}, name } = b;
                                        return <li key={i} data-id={JSON.stringify(b)}>
                                            <img src={(imgs && imgs[0])} />
                            ...
                            {(isLive || isTop) && <ul className='products' id={`item_sorter_${currModuleIndex}`}>
                                {list.map((b, i) => {
                                    let { imgs = {}, name } = b;
                                    return <li key={i} data-id={JSON.stringify(b)}>
                                        <img src={(imgs && imgs[0])} />
                            ...
                            {(isArticle||isDecoration) && <ul className='products' id={`item_sorter_${currModuleIndex}`}>
                                {list.map((b, i) => {
                                    return <li key={i} data-id={JSON.stringify(b)}>
                                        <img src={(b.imgCover || b.img)} />
                                        <div className='title'>
                                            {b.title}
                                        </div>
                                        <a className='opt' onClick={() => {
                            ...
                            {isAfflatus && <ul className='afflatus' id={`item_sorter_${currModuleIndex}`}>
                                {list.map((b, i) => {
                                    return <li key={i} data-id={JSON.stringify(b)}>
                                        <img src={(b.imgCover || b.img)} />
                                        <div className='title'>
